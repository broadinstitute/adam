...
import org.bdgenomics.adam.rdd.CompressionCodecEnum
...
import CompressionCodecEnum.CompressionCodec
...
package parquet.format.converter {
  ...
  /**
   * BECAUSE PARQUET'S DECISIONS FOR MAKING PARTICULAR METHODS PRIVATE, PROTECTED, AND PACKAGE-PROTECTED
   * IS IDIOTIC OMG.
   */
...
package org.bdgenomics.adam.rdd {
  ...
  import java.io.{ EOFException, FileInputStream, File }
  import java.nio.ByteBuffer
  import parquet.column.page.{ DictionaryPage, Page }
  import parquet.format.{ FieldRepetitionType, SchemaElement, PageType, PageHeader }
  import parquet.bytes.BytesInput
  import parquet.column.ColumnDescriptor
  import org.apache.hadoop.io.compress.{ CompressionCodec => HadoopCompressionCodec, CodecPool, Decompressor }
  ...
  object ParquetCommon {
    ...
    def readFileMetadata(io: ByteAccess): FileMetaData = {
      ...
      assert(magicAndLength.slice(4, magicAndLength.length).sameElements(MAGIC))
    ...
    def getPrimitiveTypeName(parquetType: parquet.format.Type): PrimitiveTypeName = {
      parquetType match {
        case parquet.format.Type.BOOLEAN              => PrimitiveTypeName.BOOLEAN
        case parquet.format.Type.INT32                => PrimitiveTypeName.INT32
        case parquet.format.Type.INT64                => PrimitiveTypeName.INT64
        case parquet.format.Type.INT96                => PrimitiveTypeName.INT96
        case parquet.format.Type.FLOAT                => PrimitiveTypeName.FLOAT
        case parquet.format.Type.DOUBLE               => PrimitiveTypeName.DOUBLE
        case parquet.format.Type.BYTE_ARRAY           => PrimitiveTypeName.BINARY
        case parquet.format.Type.FIXED_LEN_BYTE_ARRAY => PrimitiveTypeName.FIXED_LEN_BYTE_ARRAY
      }
    }
  ...
  object CompressionCodecEnum extends Enumeration {
    ...
    def getHadoopCodec(parquetCodec: CompressionCodec, conf: Configuration): Option[HadoopCompressionCodec] = {
      val codec = parquetCodec match {
        ...
        //Some(new com.hadoop.compression.lzo.LzoCodec())
  ...
  case class ParquetColumnChunk(numValues: Long,
    ...
    private def decompress(codec: HadoopCompressionCodec, input: InputStream, compressed: Int, uncompressed: Int): BytesInput = {
      ...
      while (read > 0 && toRead > 0) {
        ...
        assert(toRead <= bytesArray.length - (compressed - toRead), "length was > the bounds %d".format(toRead, bytesArray.length - compressed + toRead))
